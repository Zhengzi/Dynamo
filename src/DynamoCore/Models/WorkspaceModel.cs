    ...
    public abstract class WorkspaceModel : NotificationObject, ILocatable, IUndoRedoRecorderClient
    {
        ...
        private bool SaveInternal(string targetFilePath)
        {
            ...
            try
            {
                ...
                string documentPath = targetFilePath.Substring(0, targetFilePath.Length - 3);
                documentPath += "xml";

                //create the xml command to open the file
                var xmlCommand = new XmlDocument();
                XmlNode rootNode = xmlCommand.CreateElement("Commands");
                xmlCommand.AppendChild(rootNode);
                XmlAttribute attribute1 = xmlCommand.CreateAttribute("ExitAfterPlayback");
                XmlAttribute attribute2 = xmlCommand.CreateAttribute("PauseAfterPlaybackInMs");
                XmlAttribute attribute3 = xmlCommand.CreateAttribute("CommandIntervalInMs");
                ...
                XmlNode userNode = xmlCommand.CreateElement("OpenFileCommand");
                XmlAttribute attribute4 = xmlCommand.CreateAttribute("XmlFilePath");
                attribute4.Value = documentPath;
                userNode.Attributes.Append(attribute4);
                rootNode.AppendChild(userNode);

                //create the package of .dyn file
                CreatePackage(targetFilePath, xmlCommand, document);
            }
        ...
        private static void CreatePackage(string filePath, XmlDocument xmlCommand, XmlDocument document)
        {
            // Convert system path and file names to Part URIs.
            Uri partUriCommand = PackUriHelper.CreatePartUri(
                                      new Uri("commands\\openCommands.xml", UriKind.Relative));
            Uri partUriResource = PackUriHelper.CreatePartUri(
                                      new Uri("resources\\nodes.xml", UriKind.Relative));
            ...
            using (Package package =
                Package.Open(filePath, FileMode.Create))
            {
                ...
                document.Save(dataStream);
                dataStream.Flush();
                dataStream.Position = 0;
                CopyStream(dataStream, packagePartResource.GetStream());         
            }
        }
